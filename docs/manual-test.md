...
- 作業ディレクトリ: `D:\AI\AssetPipeline\asset-intake-naming-optimization-audit`
...
```powershell
cd D:\AI\AssetPipeline\asset-intake-naming-optimization-audit
npm test
npm start
```
...
5. CLI + asset manifest validator の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/asset-intake-naming-optimization-audit-docs.zip` に存在する。
